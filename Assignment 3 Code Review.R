  ...
  if (grepl("^[a-zA-Z]$", answer)) {
    ...
    if (answer %in% guessed_letters) {
      ...
      if (answer %in% strsplit(word, "")[[1]]) {
        ...
        updated_word <- "" #updated word was initialized twice
    ...
    if (answer == "guess") {
      ...
      if (guess %in% incorrect_letters) {
        ...
        if (guess %in% word) {
          ...
          # If the word has not been guessed correctly, subtract one guess from remaining guesses
          guesses_left <- guesses_left - 1
...
#Code Review
#' Overall great job! The hangman game runs flawlessly and has effective defensive coding that ensures the program can handle whatever input is typed in.
#' The use of the grepl() function very efficiently checked the user guess to ensure it was valid, along with the if statement checking if the letter was already guessed. Great job!
#' I also liked the way you allowed the user to guess the word, the code written to achieve this was very simple and effective. 
#'One recommendation I would make would be to use functions in the code as well. Although this isn't necessary, it makes the more readable and easier to decipher. 
#'In this version of the code, the line of code that informs the user of the number of letters in the word (line 18), and the code that says Welcome to Hangman(39) are not included in the game when source is pressed.
#'If you defined a function that encompassed the entire game, these lines of the code can be placed within the function, along with the variables to initialize, and then would displayed every time the game is started. The variables would also be initialized before every game. 
...
#'
#'Then, at the end of the code, you can call on the play_game() function and the entire game can be run seamlessly. 
#'Functions could have been created for other tasks in the game also, such as to check the user input or to allow the user to guess the entire word. This would not only make the code easier to read, but it would make the loop conditions less confusing with fewer lines of code within it. 
